...
const ChatPanel: React.FC<ChatPanelProps> = ({ 
  ...
  const [searchAgeMin, setSearchAgeMin] = useState('18'); // Added Min Age
  const [searchAgeMax, setSearchAgeMax] = useState('80'); // Added Max Age
  const [searchGender, setSearchGender] = useState('any'); // Added Gender
  ...
  const handleSearch = () => {
      ...
      const minAge = parseInt(searchAgeMin);
      const maxAge = parseInt(searchAgeMax);
      ...
      const results = DEMO_USERS.filter(u => {
          ...
          const matchGender = searchGender === 'any' || u.gender === searchGender;
          return matchName && matchLogin && matchAge && matchCountry && matchCity && matchGender;
      });
                ...
                <div className="flex-1 flex flex-col overflow-hidden animate-in slide-in-from-right duration-300">
                    <div className="p-6 overflow-y-auto no-scrollbar pb-20">
                        <div className="space-y-4 mb-8">
                            ...
                            <div className="grid grid-cols-2 gap-3"> {/* Changed from 3 cols to 2 for better grouping */}
                                <div>
                                    <label className="text-[10px] font-bold text-slate-500 uppercase ml-1">{t.age} ({t.from})</label>
                                    <select value={searchAgeMin} onChange={(e) => setSearchAgeMin(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-2 py-2.5 text-white text-xs outline-none appearance-none font-bold">
                                        {AGES.map(a => <option key={a} value={a} className="bg-slate-900">{a}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="text-[10px] font-bold text-slate-500 uppercase ml-1">{t.age} ({t.to})</label>
                                    <select value={searchAgeMax} onChange={(e) => setSearchAgeMax(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-2 py-2.5 text-white text-xs outline-none appearance-none font-bold">
                                        {AGES.map(a => <option key={a} value={a} className="bg-slate-900">{a}</option>)}
                                    </select>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-3"> {/* Moved gender to a new row, now 2 cols */}
                                <div>
                                    <label className="text-[10px] font-bold text-slate-500 uppercase ml-1">{t.gender}</label>
                                    <select value={searchGender} onChange={(e) => setSearchGender(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-2 py-2.5 text-white text-xs outline-none appearance-none font-bold">
                            ...
                            <div className="grid grid-cols-2 gap-3"> {/* New row for city to maintain structure */}
                                ...
                                <div></div> {/* Empty div to align layout */}
                            </div>
                        ...
                        <div className="space-y-3">
                            {searchResults.length === 0 ? (
                                <div className="text-center py-10 opacity-50"><UsersIcon className="w-12 h-12 mx-auto mb-2" /><p className="text-[10px] uppercase font-bold tracking-widest">{t.noUsers}</p><button onClick={() => { setSearchResults(DEMO_USERS); setSearchName(''); setSearchLogin(''); setSearchAgeMin('18'); setSearchAgeMax('80'); setSearchGender('any'); setSearchCountry('Any'); setSearchCity('Any'); }} className="mt-2 text-[10px] text-primary underline">{t.showAll}</button></div>
                            ) : (
